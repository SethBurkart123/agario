...
| P2 | A grid blur matched the reference edge spread but made Canvas recompute every line each frame. | Live measurement fell to 18.4 FPS with the blur and returned to 120.5 FPS without it. | The blur was removed; low-alpha subpixel strokes retain the reference contrast without the filter cost. |
